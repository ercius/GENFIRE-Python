...
if __name__ != "__main__":
    def reconstruct(numIterations, initialObject, support, measuredK, constraintIndicators,
                    constraintEnforcementDelayIndicators, R_freeInd_complex, R_freeVals_complex,
                    displayFigure, use_positivity=True, use_support=True, verbose=True):
        """
         * reconstruct *
        ...
        :param use_positivity: All values in reconstruction will be positive. Default True
        :param use_support: Use a support to set values outside known object area to zero. Default True
        :param verbose: Extra output. Default True
        ...
        # setup output dict
        if R_freeInd_complex:
            outputs = {'reconstruction': initialObject, 'errK': errK, 'R_free_bybin': Rfree_complex_bybin,
                       "R_free_total": Rfree_complex_total}
        else:
        ...
        iterationNumsToChangeCutoff = np.round(
            np.linspace(1, numIterations, num=np.size(constraintEnforcementDelayIndicators)))
        iterationNumsToChangeCutoff, uniqueIndices = np.unique(iterationNumsToChangeCutoff, return_index=True)
        iterationNumsToChangeCutoff = np.append(iterationNumsToChangeCutoff,
                                                1e30)  # add an arbitrarily large iteration number that won't be reached
        ...
        for iterationNum in range(1, numIterations + 1):  # iterations are counted started from 1
            ...
            # update current Fourier constraint if appropriate
            ...
            # compute error
            errK[iterationNum - 1] = np.sum(abs(np.abs(k[errInd]) - np.abs(measuredK[errInd]))) / np.sum(
                abs(measuredK[errInd]))  # monitor error
            if verbose:
                print("Iteration number: {0}/{1}           error = {2:0.5f}".format(iterationNum, numIterations,
                                                                                    errK[iterationNum - 1]))
            ...
            if displayFigure.DisplayFigureON:
                if iterationNum % displayFigure.displayFrequency == 0:
                    ...
                    plt.imshow(np.squeeze(
                        np.fft.fftshift(initialObject)[n_half_x, n_half_y - half_window_y:n_half_y + half_window_y,
                        n_half_z - half_window_z:n_half_z + half_window_z]))
                    plt.title("central YZ slice")

                    plt.subplot(232)
                    plt.imshow(np.squeeze(
                        np.fft.fftshift(initialObject)[n_half_x - half_window_x:n_half_x + half_window_x, n_half_y,
                        n_half_z - half_window_z:n_half_z + half_window_z]))
                    ...
                    plt.imshow(np.squeeze(
                        np.fft.fftshift(initialObject)[n_half_x - half_window_x:n_half_x + half_window_x,
                        n_half_y - half_window_y:n_half_y + half_window_y, n_half_z]))

                    plt.subplot(236)
                    plt.title("YZ projection")
                    plt.imshow(np.squeeze(np.sum(
                        np.fft.fftshift(initialObject)[n_half_x - half_window_x:n_half_x + half_window_x,
                        n_half_y - half_window_y:n_half_y + half_window_y,
                        n_half_z - half_window_z:n_half_z + half_window_z], axis=0)))
                    ...
                    plt.imshow(np.squeeze(np.sum(
                        np.fft.fftshift(initialObject)[n_half_x - half_window_x:n_half_x + half_window_x,
                        n_half_y - half_window_y:n_half_y + half_window_y,
                        n_half_z - half_window_z:n_half_z + half_window_z], axis=1)))
                    ...
                    plt.imshow(np.squeeze(np.sum(
                        np.fft.fftshift(initialObject)[n_half_x - half_window_x:n_half_x + half_window_x,
                        n_half_y - half_window_y:n_half_y + half_window_y,
                        n_half_z - half_window_z:n_half_z + half_window_z], axis=2)))
    ...
    def fillInFourierGrid(projections, angles, interpolationCutoffDistance, enforce_resolution_circle=True,
                          permitMultipleGridding=True, verbose=True):
        """
        * fillInFourierGrid *
        ...
        for projNum in range(0, numProjections):
            ...
            R = np.array([[np.cos(psi) * np.cos(theta) * np.cos(phi) - np.sin(psi) * np.sin(phi),
        ...
        for Yshift in range(-shiftMax, shiftMax + 1):
            for Xshift in range(-shiftMax, shiftMax + 1):
                for Zshift in range(-shiftMax, shiftMax + 1):
                    ...
                    distances = np.abs(measuredX - tmpX) ** 2 + np.abs(measuredY - tmpY) ** 2 + np.abs(
                        measuredZ - tmpZ) ** 2
                    tmpX += nc
                    tmpY += nc
                    tmpZ += nc

                    goodInd = (np.logical_not(
                        (tmpX > (dim1 - 1)) | (tmpX < 0) | (tmpY > (dim1 - 1)) | (tmpY < 0) | (tmpZ > (dim1 - 1)) | (
                                tmpZ < 0))) & (distances <= (interpolationCutoffDistance ** 2))

                    masterInd = np.append(masterInd, np.ravel_multi_index((tmpX[goodInd].astype(np.int64),
                                                                           tmpY[goodInd].astype(np.int64),
                                                                           tmpZ[goodInd].astype(np.int64)),
                                                                          [dim1, dim1, dim1], order='F'))
        ...
        * fillInFourierGrid_DFT *
        ...
        try:
            ...
            for proj_num in range(num_projections):
                ...
                D = pointToPlaneDistance(
                    np.vstack((K1.flatten(order="F"), K2.flatten(order="F"), K3.flatten(order="F"))).T,
                    normVECs[proj_num, :])
                Dind = np.array(np.where(D < interpolationCutoffDistance))
                CP = pointToPlaneClosest(np.vstack(
                    (K1.flatten(order="F")[[Dind]], K2.flatten(order="F")[[Dind]], K3.flatten(order="F")[[Dind]])).T,
                                         normVECs[proj_num, :].T, np.zeros(np.size(Dind), dtype=float))
                ...
                Fpoints = np.sum(curr_proj[nonzero_ind] * np.exp(
                    -1j * 2 * PI * (K10[nonzero_ind] * X / n1 + K20[nonzero_ind] * Y / n2)), axis=1)
                distances = D[Gind]
            ...
            for proj_num in range(num_projections):
                ...
                for i in range(np.shape(G_CP_plane)[1]):
                    Fpoints[i] = np.sum(curr_proj * np.exp(
                        -1j * 2 * PI * (K10_t * G_CP_plane[0, i] / n1 + K20_t * G_CP_plane[1, i] / n2)))
...
class GenfireReconstructor():
    ...
    def reconstruct(self):
        ...
        if isinstance(self.params.projections, str):
            projections = genfire.fileio.loadProjections(
                self.params.projections)  # load projections into a 3D numpy array
        else:
        ...
        if isinstance(self.params.initialObject, np.ndarray):
            ...
            if self.params.initialObject is not None and os.path.isfile(self.params.initialObject):
                ...
                initialObject = np.pad(initialObject, ((padding, padding), (padding, padding), (padding, padding)),
                                       'constant')
            else:
        ...
        if self.params.calculateRfree:
            ...
            numberOfBins = int(round(dims[
                                         0] / 2 / shell_thickness_pixels))  # number of frequency bins. Rfree will be tracked within each shell separately
            ...
            for shellNum in range(0, numberOfBins):
                ...
                measuredPointInd_complex = np.where((measuredK[:, :, 0:(np.shape(measuredK)[-1] // 2 + 1)] != 0) &
                                           (K_indicesSmall >= (spatialFrequencyForRfree[shellNum])) &
                                           (K_indicesSmall <= (spatialFrequencyForRfree[shellNum + 1])))

                # randomly shuffle
                shuffledPoints = np.random.permutation(np.shape(measuredPointInd_complex)[1])
                measuredPointInd_complex = (
                    measuredPointInd_complex[0][shuffledPoints], measuredPointInd_complex[1][shuffledPoints],
                    measuredPointInd_complex[2][shuffledPoints])
                ...
                R_freeVals_complex.append(
                    measuredK[R_freeInd_complexX[shellNum], R_freeInd_complexY[shellNum], R_freeInd_complexZ[shellNum]])
        ...
        if self.params.resolutionExtensionSuppressionState == 1:  # resolution extension/suppression
            constraintEnforcementDelayIndicators = np.array(
                np.concatenate((np.arange(0.95, -.25, -0.15), np.arange(-0.15, .95, .1)), axis=0))
